...
%% FLAG: This is where we should start making changes
...
%% FLAG: this type of testing is probably going to be overwhelmed by 
%% variance
...
s1.all <- subset(starmine, date == "1995-01-31" & !is.na(cap.usd) &
                 !is.na(sector) & !is.na(country) & !is.na(ret.1.0.m))
...
xtable(structure(s1.out[c("country", "sector", "cap.usd", "smi", "ret.1.0.m")],
...
s1.summ.lf$num <-
  rbind(data.frame(category = "Return",     val = s1.all$ret.1.0.m),
...
# Performance
perf.port <- sum(s1$ret.1.0.m * s1$weight.smi)
perf.uni  <- sum(s1$ret.1.0.m * s1$weight.uni)
perf.uni.all <- sum(s1.all$ret.1.0.m * s1.all$weight.uni)
@

The object of our analysis is a simple long-only portfolio using the SMI score,
splitting our position equally among the \Sexpr{n.port}
...
\Sexpr{formatpercent(perf.port)} over the following month.
...
\caption{Cross-sections of universe stocks by market cap and
  sector. There is one panel for each of the four countries, the United States,
  Japan, Great Britain and France, with the most universe
...
perf.match <- sum(s1$ret.1.0.m[rr.prop] * s1$weight.smi[rr.port])
@ 
...
\Sexpr{formatpercent(perf.match, digits = 2)}
versus a return of \Sexpr{formatpercent(perf.port)} for the original
...
rp.ret <- rp.return(rp.1, rr.port, s1, "ret.1.0.m", "weight.smi")
@ 
...
outperforms \Sexpr{formatpercent(mean(perf.port > rp.ret))} of the
...
\caption{Histogram showing the returns of random, matched portfolios
  ...
  \Sexpr{formatpercent(mean(perf.port > rp.ret))} percentile of the
  random returns.}
...
perf.ls <- sum(s1$weight.smi.ls * s1$ret.1.0.m)
@ 
...
perf.ls.m <- sum(s1$weight.smi.ls[rr.ls] * s1$ret.1.0.m[rr.ls.m])
@ 
...
The matched portfolio returns \Sexpr{formatpercent(perf.ls.m)}
compared with \Sexpr{formatpercent(perf.ls)} for the original
...
rp.ls.ret <- rp.return(rp.ls, rr.ls, s1, "ret.1.0.m", "weight.smi.ls")
@ 
...
return falls at the \Sexpr{formatpercent(mean(perf.ls > rp.ls.ret))}
...
\caption{Histogram showing the returns of random portfolios matched to
  ...
  stands at the \Sexpr{formatpercent(mean(perf.ls > rp.ls.ret))}
