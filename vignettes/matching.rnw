...
s1$country = factor(s1$country, levels=c("JPN", "USA", "FRA", "GBR"), labels = c("Japan", "USA", "France", "Great Britain"))
...
biased.points <- matrix(runif(30000), ncol = 3)
biased.points <- apply(biased.points, 2, function(x) x/rowSums(biased.points))
biased.points.p <- project.simplex(biased.points)
biased.plot <- ggplot(biased.points.p, aes(x=x, y=y)) + geom_point()

biased.plot <- biased.plot + simpTheme +
...
ggsave(filename="biased.jpg", plot=biased.plot)

uni.points <- matrix(rexp(30000), ncol = 3)
uni.points <- apply(uni.points, 2, function(x) x/rowSums(uni.points))
uni.points.p <- project.simplex(uni.points)
uni.plot <- ggplot(uni.points.p, aes(x=x, y=y)) + geom_point(size = 0.5)
uni.plot <- uni.plot + simpTheme +
  geom_segment(aes(x = 0, y = 0, xend = .5, yend = sqrt(3)/2)) +
  geom_segment(aes(x = .5, y = sqrt(3)/2, xend = 1, yend = 0)) + 
  geom_segment(aes(x = 1, y = 0, xend = 0, yend = 0))
ggsave(filename="unbiased.jpg", plot=uni.plot)
@

\begin{figure}[H]
  \begin{subfigure}[b]{0.3\textwidth}
      \centering
      \includegraphics[width=\textwidth]{biased}
      \caption{Biased}
      \label{fig:biased}
  \end{subfigure}
  \begin{subfigure}[b]{0.3\textwidth}
      \centering
      \includegraphics[width=\textwidth]{unbiased}
      \caption{Unbiased}
      \label{fig:unbiased}
      \end{subfigure}
  \end{figure}
...
<<mirror walk-through plots, echo = FALSE>>=
...
<<hitandrun walkthrough, echo = FALSE>>=
...
p4 <- ggplot(data = dat, aes(x = x, y = y)) + geom_point(size = 3) + 
...
p4 <- ggplot(data = dat2, aes(x = x, y = y)) + geom_point(size = 3) + 
  simpTheme +
  geom_segment(aes(x = 0, y = 0, xend = .5, yend = sqrt(3)/2)) +
  geom_segment(aes(x = .5, y = sqrt(3)/2, xend = 1, yend = 0)) + 
  geom_segment(aes(x = 1, y = 0, xend = 0, yend = 0)) + 
