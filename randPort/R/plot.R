...
## set plot for matching object
setMethod("plot",
          signature(x = "matching"),
          function(x){
              rets = returns(x)
              rg = range(rets$matching)
              binw = (rg[2]-rg[1])/30
              p = ggplot(data = data.frame(ret = rets$matching), aes(x=ret)) + geom_histogram(binwidth = binw)+ xlab("Return")
              p = p + geom_vline(x = rets$original, color = "red", linetype = "dashed", size = 2)
              p = p + ggtitle("Returns of Matched Portfolios (Red is original)")
              return(p)
