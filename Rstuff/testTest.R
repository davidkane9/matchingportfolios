...
qplot(x = Date,y = Temperatures, geom_smooth(aes(ymin = 0,ymax =
                   100)), geom = "line")

x <- read.table("link1.txt")
dates1 <- x[,1]
temps1 <- x[,3]
plot(dates1, temps1, xlab="Date", ylab="Temperature (degrees Celsius)")

y <- read.table("link2.txt", sep = "\t")
dates2 <- y[,1]
temps2 <- y[,2]
plot(dates2, temps2, xlab="Date", ylab="Temperature (degrees Celsius)")
